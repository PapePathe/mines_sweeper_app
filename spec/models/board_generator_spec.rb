...
RSpec.describe BoardGenerator do
  ...
  it 'raises errors if arguments are invalid' do
    # W: Prefer single-quoted strings when you don't need string interpolation or specia
    expect do
      instance.generate(5, 5, '5a')
    end.to raise_error(InvalidArgument)
    expect { instance.generate(5, [5], 5) }.to raise_error(InvalidArgument)
    # W: Prefer single-quoted strings when you don't need string interpolation or speci
    expect do
      instance.generate('test', 5, 5)
    end.to raise_error(InvalidArgument)
